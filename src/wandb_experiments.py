...
def run_experiment(args, run_name, log_details=False, tags=None):
    ...
    if wandb.run is not None:
        wandb.finish()
    run = wandb.init(
        project=PROJECT_NAME, name=run_name, config=config, tags=tags or []
    )
...
def section_2_2():
    ...
    def sweep_train():
        ...
        if cfg.architecture_pattern == "funnel":
            h_sizes = [
                max(32, int(cfg.hidden_size / (2**i))) for i in range(cfg.num_layers)
            ]
        else:
    ...
    if top3[0]["weights"]:
        ...
        print(
            f"  Saved best_model.npy (F1={top3[0]['test_f1']:.4f}, Acc={top3[0]['test_accuracy']:.4f})"
        )
    ...
    with open(os.path.join(BASE_DIR, "best_configs.txt"), "w") as f:
        for i, cfg in enumerate(top3_configs):
            f.write(
                f"Config {i+1} (test_f1={cfg['test_f1']:.4f}, test_acc={cfg['test_accuracy']:.4f})\n"
            )
    ...
    for i, cfg in enumerate(top3_configs):
        print(
            f"#{i+1}: F1={cfg['test_f1']:.4f} Acc={cfg['test_accuracy']:.4f} | {cfg['optimizer']} {cfg['activation']} {cfg['num_layers']}x{cfg['hidden_size']}"
        )
    print(
        "=============================================================================="
    )
...
def section_2_5():
    ...
    for act in ["relu", "tanh"]:
        for i in range(3):
            ...
            model, _, run, _, _ = run_experiment(
                args, f"dead_neuron_{act}_trial_{i+1}", log_details=True, tags=["2.5"]
            )
            run.finish()
    print(
        "=============================================================================="
...
def section_2_8():
    ...
    if not os.path.exists(best_config_path) or not os.path.exists(best_model_path):
        print(
            f"ERROR: Run section 2.2 first to generate best_model.npy and best_config.json in {BASE_DIR}"
        )
        return
    ...
    if wandb.run is not None:
        wandb.finish()
    run = wandb.init(
        project=PROJECT_NAME, name="best_model_confusion", config=cfg, tags=["2.8"]
    )
...
def section_2_10():
    ...
    if not os.path.exists(best_configs_path):
        print(
            f"ERROR: Run section 2.2 first to generate best_configs.json in {BASE_DIR}"
        )
        return
    ...
    for i, cfg in enumerate(top3):
        print(
            f"#{i+1}: {cfg['optimizer']} {cfg['activation']} {cfg['num_layers']}x{cfg['hidden_size']}"
        )
    for i, cfg in enumerate(top3):
        args = Args(
        ...
        model, best_weights, run, X_test, y_test = run_experiment(
            args, name, tags=["2.10"]
        )
